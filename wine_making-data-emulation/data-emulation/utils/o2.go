...
func InitO2(i int) float64 {
	xPoint := float64(i + 1)
	quadratic := o2_a * math.Exp(-o2_b*xPoint)
	sine := o2_amplitude1 * math.Sin(o2_frequency*xPoint+o2_phaseShift)
	noise := rand.NormFloat64() * 0.09
	res := math.Abs(quadratic + sine + noise)
	//fmt.Printf("func InitO2: xPoint=%f, quadratic=%f, sine=%f, noise=%f, res=%f\n", xPoint, quadratic, sine, noise, res)
